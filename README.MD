...
ระบบจัดการการเรียนรู้ออนไลน์ (LMS) และคลังความรู้สำหรับองค์กร (KM) ของ **Soft Inter Chiangrai Co., Ltd.** พัฒนาด้วย **Angular 22** และ `@sic-ng` Design System Framework

---

## 📑 สารบัญเอกสารสำคัญ (Key Documentation)

- 📖 [HANDOFF.md](file:///c:/Project/sicr-framework-ng/HANDOFF.md) — เอกสาร Handoff และสเปกทางเทคนิคฉบับสมบูรณ์ (Architecture, Database Schema, API Endpoints, Roadmap)
- 📌 [project.md](file:///c:/Project/sicr-framework-ng/project.md) — สรุปแหล่งอ้างอิงและลิงก์ภายนอก (Docmost / Disprz Enterprise Guide)
- 🎨 [คู่มือ Component Library (`sic-ng`)](#sic-ng-component-library-guide) — ด้านล่าง
...
### 2. รันแอปพลิเคชัน Development Server
```bash
npm start
# หรือ
ng serve
...
sicr-framework-ng/
├── projects/
│   ├── demo/                  # แอปพลิเคชัน Frontend (SICR E-Learning & Tutorial)
│   └── sic-ng/                # UI Component Library & Design System Framework
├── scripts/                   # Build & Asset synchronization scripts
├── HANDOFF.md                 # รายละเอียดสเปกระบบ E-Learning & KM
├── project.md                 # แหล่งข้อมูลอ้างอิง
├── angular.json               # การตั้งค่า Angular Workspace
└── package.json               # การจัดการ Dependencies
```

---

<a name="sic-ng-component-library-guide"></a>
## 🧩 sic-ng Component Library Guide
...
This defines every `--sic-*` design token for both light and dark mode (see [§2](#2-theming--custom-design-colors-sizes)). It also bundles the structural CSS that Angular CDK overlays need (`sic-search`, `sic-popover`, `sic-dialog`, `sic-tooltip`, and the combobox/datepicker/timepicker/phone/mention panels all popup via `@angular/cdk/overlay`) — you do **not** need to separately import `@angular/cdk/overlay-prebuilt.css`; skipping it is what causes popups to silently misposition (e.g. a "centered" panel rendering at the page's top-left instead).
...
#### `sic-gridpanel`
Editable data grid: inline cell editing (text/number/date/time/color/upload/combobox/checkbox/radio/button), row states (new/updated/deleted) with reset, row selection, sorting, a review-changes mode, a footer summary row, and pagination. It does not call `HttpClient` itself — loading and saving are delegated to output events so the host app owns the actual fetch/persist.

| Input | Type | Default |
|---|---|---|
| `config` | `SicGridPanelConfig` | — (required) |

`SicGridPanelConfig` fields: `id`, `column`/`columns` (`SicGridColumnConfig[]`), `defaultSortField`, `defaultSortDescending`, `pageable`, `lazy` (default `true` — every page/sort re-emits `loadData` for the host to fetch just that page; set `false` to fetch the whole dataset once and let the grid sort/paginate it locally with no further round-trips), `pageNumber`, `pageSize`, `keyword`, `params`, `disableRow`, `createRowValue`, `summary` (`SicGridSummaryConfig[]`, aggregates the current page per column — `sum`/`avg`/`count`/`min`/`max`/`custom`, with optional `align`).

| Output | Payload |
|---|---|
| `loadData` | `SicGridLoadRequest` — respond by calling `grid.setRows(rows, pageable?, requestId?)` or `grid.setLoadError(message, requestId?)` |
| `saveData` | `SicGridSaveRequest` — respond by calling `grid.setSaveResult(success, message?, requestId?)` |
| `rowsChange` | `SicGridRowData[]` |
| `rowAction` | `{ action, row?, rows?, column? }` |
| `softDeleteChange` | `{ id, deleted, row }` |

Public methods: `reload()`, `setRows()`, `setLoadError()`, `setSaveResult()`.
